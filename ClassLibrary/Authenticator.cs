...
namespace ClassLibrary
{
    public class Authenticator
    {
        ...
        {
            var personID = "";
            if (userId == "farzanajahan" || userId == "astellav")
            {                
                if (userId == "farzanajahan")
                {
                    personID = "2863569";
                }
                else if (userId == "astellav")
                {
                    personID = "2835256";
                }
                var ar = new AuthenticateResult() { ExtensionAttribute13 = personID };
                ar.MemberOf.Add("Developer");
